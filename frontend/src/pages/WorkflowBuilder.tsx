...
function FlowCanvas() {
  ...
  const onConnect = useCallback(
    (params: Connection) => setEdges((eds) => addEdge({ ...params, animated: true, style: { stroke: '#5A7D9A' } }, eds)),
    [setEdges]
  );
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      const newNode: Node = {
        ...
        type,
        position,
        data: { label: `${type.charAt(0).toUpperCase() + type.slice(1)} Node`, agent_type: 'executor', condition: '' },
      };
  ...
  const executeWorkflow = async () => {
    try {
      ...
      const result = await apiClient.executeWorkflow(workflowName, '');
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] -m-8">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left Palette */}
        <div className="w-56 border-r border-outline/10 bg-surface-low p-4 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
              ...
              {selectedNode.type === 'agent' && (
                <div>
                  ...
                  <select
                    className="obsidian-input text-sm w-full"
                    value={selectedNode.data.agent_type || 'executor'}
              ...
              {selectedNode.type === 'wait' && (
                <div>
                  ...
                  <textarea
                    ...
                    value={selectedNode.data.approval_config ? JSON.stringify(selectedNode.data.approval_config) : ''}
                    onChange={(e) => {
                      try {
