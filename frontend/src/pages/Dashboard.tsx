...
const Dashboard = () => {
  ...
  const currentTraceSpans = taskTrace?.spans ?? [];
      ...
      <motion.div variants={itemVariants} className="obsidian-panel border border-outline/10 p-6">
        ...
        {currentTask && (
          <div className="mt-4 p-4 bg-surface-highest rounded-lg border border-outline/10">
            ...
            {currentTask.steps && currentTask.steps.length > 0 && (
              <div className="mt-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {currentTask.steps.map((step: any, idx: number) => (
                    <div key={step.step_id || idx} className="flex flex-col gap-2 px-3 py-3 bg-surface-low rounded-lg text-xs border border-outline/10">
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2 min-w-0">
                          <ChevronRight className="w-3 h-3 text-primary shrink-0" />
                          <span className="text-secondaryText truncate">{step.step_id || `Node ${idx + 1}`}</span>
                        </div>
                        <span className={`font-bold uppercase ${step.status === 'FAILED' ? 'text-[#FF4B4B]' : step.status === 'SKIPPED' ? 'text-secondaryText' : 'text-[#00FF88]'}`}>
                          {step.status || 'PENDING'}
                        </span>
                      </div>
                      <div className="text-secondaryText">
                        <span className="font-medium text-primaryText">Dependency:</span> {Array.isArray(step.depends_on) && step.depends_on.length > 0 ? step.depends_on.join(', ') : 'None'}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {taskTrace?.node_traces && taskTrace.node_traces.length > 0 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={itemVariants} className="obsidian-panel border border-outline/5 p-6 flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4">
            ...
            {taskTrace?.workflow_state?.nodes && taskTrace.workflow_state.nodes.length > 0 && (
