...
class WorkflowMode(ModeStrategy):
    ...
    async def execute(self, runtime, orchestrator, query, config, task_id, user_id):
        ...
        # Attempt to load a predefined workflow by ID from config
        workflow_id = config.get("workflow_id")
        if workflow_id:
            predefined = await workflow_repo.get_by_id(workflow_id)
            if predefined:
                logger.info(f"Loaded predefined workflow '{workflow_id}' for task {task_id}")
                ...
                if result.status == AgentStatus.SUCCESS and result.output_data:
                    ...
                    result.output_data["workflow_id"] = workflow_id
                return result

        # Fall back to dynamic planning
        logger.info(f"No predefined workflow found for '{workflow_id}', falling back to dynamic plan")
        result = await orchestrator._execute_pipeline(query, config, task_id, user_id)
        if result.status == AgentStatus.SUCCESS and result.output_data:
            result.output_data["mode"] = "workflow"
        return result
