              ...
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentTask.steps.map((step: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-2 px-3 py-1 bg-surface-low rounded-full text-xs">
                      <ChevronRight className="w-3 h-3 text-primary" />
                      <span className="text-secondaryText">{step.agent || `Step ${idx + 1}`}</span>
                    </div>
                  ))}
                </div>
              </div>
