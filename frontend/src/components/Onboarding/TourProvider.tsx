...
export const TourProvider: React.FC<TourProviderProps> = ({ tourId, steps, onComplete }) => {
 ...
 className="bg-surface-high border border-outline/20 rounded-none shadow-pixel p-4 w-80 text-primaryText"
 style={tooltipStyle}
 >
 ...
 onClick={handleSkip}
 whileTap={{ scale: 0.96 }}
 className="px-4 py-2 text-xs rounded-none text-secondaryText hover:bg-surface-highest "
 >
 Skip
 </motion.button>
 <motion.button
 onClick={handleNext}
 whileTap={{ scale: 0.96 }}
 className="px-4 py-2 text-xs rounded-none bg-primary text-background font-medium hover:brightness-110 flex items-center gap-1"
 >
