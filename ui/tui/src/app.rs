...
impl App {
    ...
    pub async fn check_connection(&mut self) -> bool {
        match agentos_ipc_client::KernelClient::connect().await {
    ...
    pub fn on_key(&mut self, key: KeyCode) {
        match key {
            ...
            KeyCode::Char('?') | KeyCode::Char('/') | KeyCode::Char('h') => {
    ...
    fn draw_dashboard(&self) -> Paragraph<'_> {
        let text = vec![
            Line::from(vec![Span::styled(
                "AgentOS TUI - Real-time Monitoring",
                Style::default()
                    .fg(Color::Cyan)
            ...
            Line::from(vec![
                Span::raw("Connection: "),
                if self.connected {
                    Span::styled("● Connected (gRPC)", Style::default().fg(Color::Green))
                } else {
            ...
            Line::from(vec![
                ...
                Span::styled(
                    self.status.active_tasks.to_string(),
...
pub fn run() -> io::Result<()> {
    ...
    // Create a tokio runtime for async operations in the sync event loop
    ...
    loop {
        ...
        // Tick: check kernel connection via gRPC
        if last_tick.elapsed() >= tick_rate {
            app.connected = rt.block_on(app.check_connection());
