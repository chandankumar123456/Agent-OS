"""AgentOS Cloud/HTTP FastAPI Application.

This module provides the HTTP API and WebSocket interface for AgentOS
when deployed as a cloud/web service. It is NOT used in desktop-native mode.

For desktop-native mode, use `app.desktop_entry` instead.

Usage:
    # Development
    python -m uvicorn app.cloud_api.main:app --host 0.0.0.0 --port 8000 --reload

    # Production
    python -m uvicorn app.cloud_api.main:app --host 0.0.0.0 --port 8000 --workers 4
"""
...
from app.orchestrator.errors import AgentOSError, ErrorCode
